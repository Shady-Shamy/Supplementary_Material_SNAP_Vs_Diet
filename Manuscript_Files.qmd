...
bns_combined_imputed_df$SNAP3 <- recode(bns_combined_imputed_df$SNAP3, "SNAP" = "SNAP Recipients", "Likely Not Eligible" = "Income-Ineligible Non-Recipients",
"No SNAP but Likely Eligible" = "Likely Eligible Non-Recipients")

# Set figure labels
figure_labels <- c("SNAP Recipients" = "SNAP Recipients (n = 739)", 
              "Income-Ineligible Non-Recipients" = "Income-Ineligible Non-Recipients (n = 449)", 
              "Likely Eligible Non-Recipients" = "Likely Eligible Non-Recipients (n = 1,174)")


bns_combined$SNAP3 <- recode(bns_combined$SNAP3, "SNAP" = "SNAP Recipients", "Likely Not Eligible" = "Income-Ineligible Non-Recipients",
"No SNAP but Likely Eligible" = "Likely Eligible Non-Recipients")
...
demographics_cc <- bns_combined %>% select(age, gender, race_eth, monthly_hh_income, academic_level, Cohort, school2, SNAP3, SNAP_eligible, cooking_confident, health_conscious, physical_activity, ssb_cat, ff_cat, fruits3, food_insecure_2item,  units, paid_work_hours)
...
demographics_imp <- bns_combined_imputed_df %>% select(age, gender, race_eth, monthly_hh_income, academic_level, Cohort, school2, SNAP3, SNAP_eligible, cooking_confident, health_conscious, physical_activity, ssb_cat, ff_cat, fruits3, food_insecure_2item,  units, paid_work_hours)
...
ff.ordinal.mod <- with(bns_imputed_mids, MASS::polr(ff_cat~SNAP3 + Cohort + gender + race_eth + age + physical_activity + storage_room + kitchen_access + health_conscious + cooking_confident + food_insecure_2item + paid_work_hours_tens + living_with_family + homelessness_year, Hess=TRUE)) %>% tbl_regression(exponentiate = TRUE) %>% bold_p()
...
ssb.ordinal.mod <- with(bns_imputed_mids, MASS::polr(ssb_cat~SNAP3 + Cohort + gender + race_eth + age + physical_activity + health_conscious + storage_access + units_four + paid_work_hours_tens + caregiving_hours_tens + homelessness_year, Hess=TRUE)) %>% tbl_regression(exponentiate = TRUE) %>% bold_p()
...
FV.ordinal.mod <- with(bns_imputed_mids, MASS::polr(fruits3~SNAP3 + Cohort + gender + race_eth + age + health_conscious + cooking_confident + physical_activity + storage_room + food_insecure_2item, Hess=TRUE)) %>% tbl_regression(exponentiate = TRUE) %>% bold_p()
