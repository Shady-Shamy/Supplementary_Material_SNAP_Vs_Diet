...
> This page shows all steps taken for variable selection. Potentially relevant variables from the survey were identified based on a theoretical approach and previous qualitative studies among college students. A LASSO regression was first used to identify which variables are relevant to each model. A ten-fold cross validation was used to determine the minimum lambda value. General linear regression models were then fit for each dependent variable including all potential covariates using a LASSO penalty. Academic level was not included as a potential covariate since the pilot 2 cohort included no freshmen and sophomores.
...
> 0.1 appeared to be a reasonable cutoff.
> Based on this cutoff, covariates will include homelessness, physical activity, storage room, kitchen access, health consciousness, paid work hours, food insecurity, living with family, cooking confidence, gender, race/eth, and Cohort. 
> Gender, age, race/eth, and cohort were included in all models regardless of LASSO results
...
> Kitchen Access had a high p value in the full model and can be removed without changing any results. No other covariates seemed like they can be removed.
...
> Living with family had a high p value in the full model and can be removed without changing any results. 
> No other covariates seemed like they can be removed.
...
> No variables in the F&V model seemed like they can be removed
:::
