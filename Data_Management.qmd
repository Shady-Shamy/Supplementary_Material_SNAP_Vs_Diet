...
> SNAP status was created using five questions, whether they are currently using SNAP, household size, monthly household income, U.S. citizenship, and lawful permanent residency if not a U.S. citizen. Those that reported currently using SNAP were coded as "SNAP". Those that met the SNAP eligibility criteria discussed in the methods sections and did not receive SNAP benefits were coded as "No SNAP but likely eligible." Those that did not meet SNAP eligibility criteria and did not receive SNAP benefits were coded as "No SNAP and likely ineligible."
...
bns$SNAP3 <- case_when(bns$current_SNAP_temp == "SNAP" ~ "SNAP", 
...
bns$SNAP3 <- factor(bns$SNAP3, levels = c("SNAP", "Likely Eligible Non-Recipient", "Income-Ineligible Non-Recipient"))
...
BNSp2b$SNAP3 <- case_when(BNSp2b$current_SNAP_temp == "SNAP" ~ "SNAP", 
