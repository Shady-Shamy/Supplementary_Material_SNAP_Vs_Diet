...
ff.ordinal.mod <- with(bns_imputed_mids, MASS::polr(ff_cat~SNAP3 + school2 + gender + race_eth + age + physical_activity + storage_room + kitchen_access + health_conscious + cooking_confident + food_insecure_2item + paid_work_hours_tens + living_with_family + homelessness_year, Hess=TRUE)) %>% tbl_regression(exponentiate = TRUE) %>% bold_p()

ssb.ordinal.mod <- with(bns_imputed_mids, MASS::polr(ssb_cat~SNAP3 + school2 + gender + race_eth + age + physical_activity + health_conscious + storage_access + units_four + paid_work_hours_tens + caregiving_hours_tens + homelessness_year, Hess=TRUE)) %>% tbl_regression(exponentiate = TRUE) %>% bold_p()

FV.ordinal.mod <- with(bns_imputed_mids, MASS::polr(fruits3~SNAP3 + school2 + gender + race_eth + age + health_conscious + cooking_confident + physical_activity + storage_room + food_insecure_2item, Hess=TRUE)) %>% tbl_regression(exponentiate = TRUE) %>% bold_p()
